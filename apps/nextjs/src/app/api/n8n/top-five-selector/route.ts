import { NextRequest, NextResponse } from "next/server";
import { z } from "zod";

const N8N_WEBHOOK_URL = process.env.N8N_TOP_FIVE_SELECTOR_URL;
const N8N_API_KEY = process.env.N8N_API_KEY;

// Early validation of required environment variables
if (!N8N_WEBHOOK_URL) {
  console.error("[N8N Top-5 Selector] N8N_TOP_FIVE_SELECTOR_URL environment variable is not set");
}
if (!N8N_API_KEY) {
  console.warn("[N8N Top-5 Selector] N8N_API_KEY environment variable is not set - requests may fail");
}

// Competitor schema (matches frontend format)
const CompetitorSchema = z.object({
  name: z.string(),
  tagline: z.string(),
  website: z.string(),
  lastUpdate: z.string(),
  confidence: z.number(),
});

// Input schema: all competitors + analysis data
const InputSchema = z.object({
  competitors: z.array(CompetitorSchema),
  analysisData: z.object({
    "Clear Requirement Statement": z.string(),
    Certainties: z.object({
      "Target User Profile": z.string().optional(),
      "Target Market": z.string().optional(),
      "Must-Haves": z.array(z.string()),
      "Success Criteria": z.array(z.string()).optional(),
      "Out of Scope": z.array(z.string()).optional(),
    }),
    "Key Assumptions": z.array(
      z.object({
        assumption: z.string(),
        rationale: z.string(),
        confidence: z.number(),
      }),
    ),
  }),
});

// N8N response schema - matches actual N8N output format
const N8NCompetitorSchema = z.object({
  product_name: z.string(),
  url: z.string(),
  description: z.string(),
  confidence: z.number(),
});

// N8N always returns an array for Top-5 Selector
const N8NResponseSchema = z.array(N8NCompetitorSchema);

export async function POST(request: NextRequest) {
  try {
    console.log("[N8N Top-5 Selector] Received request");
    console.log("[N8N Top-5 Selector] Webhook URL:", N8N_WEBHOOK_URL);
    console.log("[N8N Top-5 Selector] API Key configured:", !!N8N_API_KEY);

    // Check if required environment variables are set
    if (!N8N_WEBHOOK_URL) {
      console.error("[N8N Top-5 Selector] Missing N8N_TOP_FIVE_SELECTOR_URL");
      return NextResponse.json(
        {
          error: "Server configuration error",
          details: "N8N_TOP_FIVE_SELECTOR_URL environment variable is not configured"
        },
        { status: 500 }
      );
    }

    const body = await request.json();
    console.log("[N8N Top-5 Selector] Request body:", JSON.stringify(body, null, 2));

    // Validate input
    const validatedInput = InputSchema.parse(body);
    console.log("[N8N Top-5 Selector] Input validated successfully");
    console.log(`[N8N Top-5 Selector] Selecting top 5 from ${validatedInput.competitors.length} competitors`);

    // Prepare payload for n8n
    const payload = {
      competitors: validatedInput.competitors,
      requirementStatement: validatedInput.analysisData["Clear Requirement Statement"],
      certainties: {
        targetUserProfile: validatedInput.analysisData.Certainties["Target User Profile"],
        targetMarket: validatedInput.analysisData.Certainties["Target Market"],
        mustHaves: validatedInput.analysisData.Certainties["Must-Haves"],
        successCriteria: validatedInput.analysisData.Certainties["Success Criteria"],
        outOfScope: validatedInput.analysisData.Certainties["Out of Scope"],
      },
      keyAssumptions: validatedInput.analysisData["Key Assumptions"],
    };

    // Call n8n webhook
    const headers: Record<string, string> = {
      "Content-Type": "application/json",
    };

    if (N8N_API_KEY) {
      headers["Authorization"] = `Bearer ${N8N_API_KEY}`;
    }

    console.log("[N8N Top-5 Selector] Calling webhook:", N8N_WEBHOOK_URL);
    console.log("[N8N Top-5 Selector] Payload:", JSON.stringify(payload, null, 2));

    // Add timeout to prevent hanging
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 60000); // 60 second timeout

    try {
      const response = await fetch(N8N_WEBHOOK_URL, {
        method: "POST",
        headers,
        body: JSON.stringify(payload),
        signal: controller.signal,
      });
      clearTimeout(timeoutId);

      if (!response.ok) {
        const errorText = await response.text();
        console.error("[N8N Top-5 Selector] Webhook error:", errorText);
        return NextResponse.json(
          {
            error: `n8n webhook failed: ${response.status} ${response.statusText}`,
            details: errorText
          },
          { status: response.status }
        );
      }

      const responseText = await response.text();
      console.log("[N8N Top-5 Selector] N8N raw response:", responseText);
      console.log("[N8N Top-5 Selector] Response length:", responseText.length);
      console.log("[N8N Top-5 Selector] Response type:", typeof responseText);

      // Check if response is empty
      if (!responseText || responseText.trim() === '') {
        console.error("[N8N Top-5 Selector] Empty response from N8N");
        console.error("[N8N Top-5 Selector] Response headers:", JSON.stringify([...response.headers.entries()]));
        return NextResponse.json(
          {
            error: "N8N workflow returned empty response",
            details: "The top-five-selector webhook may not be properly configured in N8N. Please check: 1) Workflow is activated, 2) 'Respond to Webhook' node is configured, 3) Response data format"
          },
          { status: 500 }
        );
      }

      let data;
      try {
        data = JSON.parse(responseText);
        console.log("[N8N Top-5 Selector] N8N response:", data);
      } catch (parseError) {
        console.error("[N8N Top-5 Selector] Failed to parse N8N response:", parseError);
        console.error("[N8N Top-5 Selector] Raw response text:", responseText);
        return NextResponse.json(
          {
            error: "Invalid JSON response from N8N",
            details: responseText.substring(0, 500) // Return first 500 chars for debugging
          },
          { status: 500 }
        );
      }

      // Validate response structure
      try {
        const validatedData = N8NResponseSchema.parse(data);
        console.log("[N8N Top-5 Selector] Validation successful");
        console.log("[N8N Top-5 Selector] Validated competitors count:", validatedData.length);

        // Convert N8N format to frontend format
        const competitors = validatedData.map((comp) => ({
          name: comp.product_name,
          tagline: comp.description,
          website: comp.url,
          lastUpdate: new Date().toISOString().split('T')[0], // Use current date
          confidence: comp.confidence,
        }));

        const response = {
          competitors: competitors,
          selectionStrategy: "AI-powered intelligent selection based on requirement alignment",
          totalFound: competitors.length,
        };

        console.log("[N8N Top-5 Selector] Returning data:", JSON.stringify(response, null, 2));
        return NextResponse.json(response);
      } catch (validationError) {
        console.error("[N8N Top-5 Selector] Validation error:", validationError);
        console.error("[N8N Top-5 Selector] Data that failed validation:", JSON.stringify(data, null, 2));
        return NextResponse.json(
          {
            error: "Invalid response format from N8N",
            details: validationError instanceof Error ? validationError.message : String(validationError)
          },
          { status: 500 }
        );
      }
    } catch (fetchError) {
      clearTimeout(timeoutId);
      if (fetchError instanceof Error && fetchError.name === 'AbortError') {
        console.error("[N8N Top-5 Selector] Request timed out");
        return NextResponse.json(
          { error: "N8N webhook request timed out after 60 seconds" },
          { status: 504 }
        );
      }
      console.error("[N8N Top-5 Selector] Fetch error:", fetchError);
      console.error("[N8N Top-5 Selector] Webhook URL was:", N8N_WEBHOOK_URL);
      return NextResponse.json(
        {
          error: "Failed to connect to N8N webhook",
          message: fetchError instanceof Error ? fetchError.message : String(fetchError),
          webhookUrl: N8N_WEBHOOK_URL.replace(/\/\/[^@]+@/, '//***@') // Hide credentials if any
        },
        { status: 502 }
      );
    }
  } catch (error) {
    console.error("[N8N Top-5 Selector] Top-level error:", error);
    console.error("[N8N Top-5 Selector] Error stack:", error instanceof Error ? error.stack : "No stack trace");

    // Check if it's a Zod validation error on input
    if (error instanceof z.ZodError) {
      return NextResponse.json(
        {
          error: "Invalid input data",
          details: error.errors,
        },
        { status: 400 }
      );
    }

    return NextResponse.json(
      {
        error: "Failed to select top 5 competitors",
        message: error instanceof Error ? error.message : "Unknown error"
      },
      { status: 500 }
    );
  }
}
